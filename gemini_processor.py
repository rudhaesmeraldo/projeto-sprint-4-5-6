...
def analisar_com_gemini(texto_extraido: str) -> dict: # chama a API do Gemini
    ...
    -> REGRAS ESTRITAS
    1.  Campos Obrigatórios: O JSON deve conter EXATAMENTE os seguintes campos: 
    ...
    2.  Formato CNPJ/CPF: O CNPJ deve ser formatado como "XX.XXX.XXX/XXXX-XX". 
        O CPF deve ser "XXX.XXX.XXX-XX".
    ...
    5.  Forma de Pagamento: O campo "forma_pgto" deve ser apenas uma das seguintes opções: 
        "dinheiro", "pix", "cartao", ou "outros".
    6.  Valores Não Encontrados: Se um campo não for encontrado no texto, o valor correspondente no JSON deve ser o valor `None`.
    7.  Formato da Resposta: Sua resposta deve ser APENAS o objeto JSON. 
        Não inclua ```json, nem explicações, nem comentários extras.

    -> EXEMPLOS DE TREINAMENTO (apenas guia, NÃO devolver junto na resposta)
    ...
    try:
        ...
        if hasattr(e, 'read'):
            print(f"Detalhes do erro da API: {e.read().decode()}")
        return None
